# Hi there 👋

I'm **Jakub**, a 17-year-old developer from Poland. I've been working with computer science for 9 years, focusing on modern web technologies, backend development, and technical research.  
I develop systems, tools, and services at **VEX Systems**.

## What I Do

- Frontend: Next.js, Vite, Vanilla HTML/CSS/JS  
- Backend: JavaScript, TypeScript, Python  
- Database experience: PostgreSQL, MySQL, MongoDB, SQLite  
- Practical work with networking hardware, server configuration, and network engineering 
- Technical research on software, security, automation, and infrastructure  
- Occasionally design and create simple, clean logos and visual concepts  
- Building and maintaining projects inside **VEX Systems**  
  - GitHub: https://github.com/VEX-Systems  
  - Website: https://vex.systems  

## WakaTime
<a href="https://wakatime.com/vloud"><img height="480" src="https://wakatime.com/share/@vLoud/dba01482-9e64-4b06-a706-9ab4fd7d18dc.png" /></a><br>
[![wakatime](https://wakatime.com/badge/user/32207571-bf9e-4f16-86e2-83acbbba16b3.svg)](https://wakatime.com/@32207571-bf9e-4f16-86e2-83acbbba16b3)

## Links

- GitHub: https://github.com/l1806-vex (you are here now)  
- WakaTime: https://wakatime.com/@vLoud

## Contact

- Email: hello@vex.systems
